...
for backend_name in ["llama3", "qwen", "deepseek"]:
    ...
    for character in character_list:
        for trial in range(1, 4):  # Run 3 times per character
            output_dir = ROOT_DIR / "evaluation" / "character_response" / "MBTI" / "myRAG" / f"{character}" / backend_name
            response_path = output_dir / f"{character}_{trial}_16p_responses.json"
            trace_path = output_dir / f"{character}_{trial}_16p_detailed_logs.json"
            ...
            for qid_str, q in questions.items():
                qid = int(qid_str)
