...
def log_to_jsonl(log_entry, log_file="outputs/train_encoder_va/train_encoder_va.jsonl"):
...
def train(model, train_loader, val_loader, contrastive_loss_fn, device, total_steps=10000, log_interval=10):
    ...
    # Combine encoder + projection head parameters
    ...
    while step < total_steps:
        for batch in tqdm(train_loader, desc=f"Training Step {step + 1}", ncols=100):
            ...
            # Validate every `log_interval` steps
            if (step + 1) % log_interval == 0:
                ...
                if val_macro_precision > best_macro_precision:
                    ...
                    model_path = "outputs/train_encoder_va/best_model.pt"
    ...
    logging.info(f"Best model saved to 'outputs/train_encoder_va/best_model.pt'")
