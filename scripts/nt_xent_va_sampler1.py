...
class ProportionalCoverageSampler(Sampler):
    def __init__(self, dataset, batch_size, num_classes, labels):
        self.dataset = dataset  # This allows it to be used with HuggingFace Trainer or create_dataloader
        ...
        # Proportional sampling (frequent classes sampled more)
    ...
    def __iter__(self):
        while True:
            ...
            # Fill remaining batch space proportionally
            while len(batch_indices) < self.batch_size:
                c = np.random.choice(self.num_classes, p=self.class_probs)
                idx = self._get_next_index(c)
                if idx is not None and idx not in batch_indices:
                    batch_indices.add(idx)
...
def log_to_jsonl(log_entry, log_file="outputs/nt_xent_va_sampler1/nt_xent_va_sampler1.jsonl"):
        ...
        while step < total_steps:
            ...
            if (step + 1) % 10 == 0:
                ...
                if val_macro_precision > best_macro_precision:
                    ...
                    os.makedirs("outputs/nt_xent_va_sampler1", exist_ok=True)
                    torch.save(model.projection_head.state_dict(), "outputs/nt_xent_va_sampler1/best_projection_head.pt")
