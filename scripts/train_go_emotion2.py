...
set_seed(11711)
base_path = Path.cwd()
model_checkpoint_path = base_path / "outputs" / "carer_roberta" / "checkpoint-4100"
...
training_args = TrainingArguments(
    output_dir=str(base_path / "outputs" / "goemotions_transfer2"),
    evaluation_strategy="steps",
    eval_steps=50,
    save_strategy="steps",
    save_steps=50,
    save_total_limit=1,
    logging_dir=str(base_path / "outputs" / "goemotions_transfer2" / "logs"),
    logging_steps=10,
    learning_rate=2e-5,  # <-- updated from 5e-5 to 2e-5
    per_device_train_batch_size=16,
    per_device_eval_batch_size=16,
    num_train_epochs=10,
    weight_decay=0.01,   # <-- already correct
    seed=11711,
...
multi_metric_callback = MultiMetricSaverCallback(
    base_dir=base_path / "outputs" / "goemotions_transfer2",
...
trainer = Trainer(
    ...
    callbacks=[
        RealTimeLoggerCallback(base_path / "outputs" / "goemotions_transfer2" / "loss_log.jsonl"),
...
with open(base_path / "outputs" / "goemotions_transfer2" / "test_results.json", "w") as f:
