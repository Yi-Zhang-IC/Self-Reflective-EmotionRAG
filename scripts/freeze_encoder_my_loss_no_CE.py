...
def log_to_jsonl(log_entry, log_file="outputs/my_loss_no_CE/freeze_encoder_my_loss_no_CE.jsonl"):
        ...
        for batch in tqdm(train_loader, desc=f"Training Step {step + 1}", ncols=100):
            ...
            if (step + 1) % 10 == 0:
                ...
                if val_macro_precision > best_macro_precision:
                    ...
                    torch.save(model.projection_head.state_dict(), f"outputs/my_loss_no_CE/best_projection_head_freeze_encoder.pt")  # Save projection head based on macro precision
