...
def log_to_jsonl(log_entry, log_file="outputs/freeze_encoder_va/freeze_encoder_va.jsonl"):
        ...
        for batch in tqdm(train_loader, desc=f"Training Step {step + 1}", ncols=100):
            ...
            if (step + 1) % 10 == 0:
                ...
                if val_macro_precision > best_macro_precision:
                    ...
                    torch.save(model.projection_head.state_dict(), f"outputs/freeze_encoder_va/best_projection_head.pt")  # Save projection head based on macro precision
