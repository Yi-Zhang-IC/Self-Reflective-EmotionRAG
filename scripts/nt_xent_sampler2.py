...
def log_to_jsonl(log_entry, log_file="outputs/nt_xent_sampler2/nt_xent_sampler2.jsonl"):
        ...
        for batch in tqdm(train_loader, desc=f"Training Step {step + 1}", ncols=100):
            ...
            if (step + 1) % 10 == 0:
                ...
                if val_macro_precision > best_macro_precision:
                    ...
                    torch.save(model.projection_head.state_dict(), f"outputs/nt_xent_sampler2/best_projection_head.pt")  # Save projection head based on macro precision
...
contrastive_loss_fn = NTXentLoss(temperature=0.07).to(device)  # Contrastive loss for projection head
