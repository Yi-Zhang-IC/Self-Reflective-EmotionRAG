...
def log_to_jsonl(log_entry, log_file="outputs/NT_XENT/freeze_encoder_NT_Xent_loss.jsonl"):
...
def train(model, train_loader, val_loader, contrastive_loss_fn, device, total_steps=10000, log_interval=10):
    ...
    while step < total_steps:
        for batch in tqdm(train_loader, desc=f"Training Step {step + 1}", ncols=100):
            ...
            if (step + 1) % 10 == 0:
                val_macro_precision, _ = evaluate_embeddings_macro_precision(model, val_loader.dataset, device)
                if val_macro_precision > best_macro_precision:
                    best_macro_precision = val_macro_precision
                    torch.save(model.projection_head.state_dict(), f"outputs/NT_XENT/best_projection_head_freeze_encoder.pt")
                    logging.info(f"Saved best projection head at step {step + 1}")
...
model = EmotionEmbeddingModel(dropout_rate=0.3, projection_dim=128, model_path=model_path, freeze_encoder=True).to(device)
