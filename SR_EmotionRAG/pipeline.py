...
def multistep_rag_loop_two_stage(
    ...
    for attempt in range(max_retries + 1):
        ...
        if isinstance(output, list) and all(isinstance(q, dict) and "query" in q and "retrieval_type" in q for q in output):
    ...
    for q in stage1_queries:
        ...
        for item in results:
            ...
            if para_idx is not None and para_idx not in seen_indices:
                ...
                step_memory_map[step_key].append(mem)
    ...
    return obtained_memories, reasoning_trace, retrieval_counts, step_memory_map
...
def run_full_roleplay_pipeline(
    ...
    max_retries: int = 5,
    retry_delay: float = 1.0,
) -> Tuple[str, str, List[Dict], Dict[str, int], Dict[str, List[Dict]]]:
    import json
    import time
    ...
    from SR_EmotionRAG.pipeline import multistep_rag_loop_two_stage

    # Step 1–2: Multi-step memory retrieval and reasoning trace
    final_memories, reasoning_trace, retrieval_counts, retrieved_memory_by_step = multistep_rag_loop_two_stage(
        character=character,
    ...
    # Step 3: Roleplay prompt construction
    ...
    # Step 4: Final character response generation
    for attempt in range(max_retries + 1):
        try:
    ...
    return roleplay_prompt, character_response, reasoning_trace, retrieval_counts, retrieved_memory_by_step
